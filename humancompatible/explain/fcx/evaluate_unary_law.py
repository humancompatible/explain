...
def evaluate_law_unary(
    base_data_dir: str = 'data/',
    base_model_dir: str = 'models/',
    dataset_name: str = 'law',
    pth_name: str = 'law-margin-0.364-feasibility-192.0-validity-29.0-epoch-25-fcx-unary.pth'
) -> dict:
    """
    Compute evaluation metrics for the Law unary FCX-VAE model.
    ...
    res[dataset_name]['validity'] = compute_eval_metrics(
        ...
        div_case, 0, sample_range, 'law-validity'
    )
    res[dataset_name]['const-score'] = compute_eval_metrics(
        immutables, methods, base_model_dir, encoded_size,
        pred_model, vae_test_dataset, d,
        normalise_weights, mad_feature_weights,
        div_case, 1, sample_range, 'law-feasibility-score'
    )
    res[dataset_name]['cont-prox'] = compute_eval_metrics(
        immutables, methods, base_model_dir, encoded_size,
        pred_model, vae_test_dataset, d,
        normalise_weights, mad_feature_weights,
        div_case, 3, sample_range, 'law-cont-proximity-score'
    )
    res[dataset_name]['cat-prox'] = compute_eval_metrics(
        immutables, methods, base_model_dir, encoded_size,
        pred_model, vae_test_dataset, d,
        normalise_weights, mad_feature_weights,
        div_case, 4, sample_range, 'law-cat-proximity-score'
    )
    res[dataset_name]['LOF'] = compute_eval_metrics(
        immutables, methods, base_model_dir, encoded_size,
        pred_model, vae_test_dataset, d,
        normalise_weights, mad_feature_weights,
        div_case, 8, sample_range, 'law-lof', prefix_name=prefix_name
    )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pth_name',       type=str, default='law-margin-0.364-feasibility-192.0-validity-29.0-epoch-25-fcx-unary.pth')
