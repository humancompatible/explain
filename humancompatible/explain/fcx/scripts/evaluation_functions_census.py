...
encoded_feature_names=['education_Children',
'education_Less than 1st grade',
'education_1st 2nd 3rd or 4th grade',
...
def compute_eval_metrics(immutables, methods, base_model_dir, encoded_size, pred_model, val_dataset, d, normalise_weights, mad_feature_weights, div_case, case, sample_range, filename,prefix_name='test'):   
    ...
    np.save('census-visualise-sample.npy', x_sample)  
    ...
    for key in methods.keys():
        ...
        if immutables:
            fcx_vae = FCX_VAE( len(d.encoded_feature_names)-7, encoded_size, d,-7 )
        else:
    ...
    os.makedirs('results/census', exist_ok=True)
    plt.savefig('results/census/'+filename+'.jpg')
    plt.show()
    print(cf_val)
    return final_res
