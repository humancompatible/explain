...
encoded_feature_names=['education_1 or more years of college credit but no degree', 
...
education_score = {
    ...
    'Master’s degree': 2,
    '1 or more years of college credit but no degree': 1,
    'Bachelor’s degree': 1,
    'Some college but less than 1 year': 1,
    'GED or alternative credential': 1,
...
"""
encoded_feature_names=['education_Assoc',
       'education_Bachelors', 
       'education_Doctorate', 
       'education_HS-grad',
       'education_Masters', 
...
def causal_score_age_ed_constraint_lof(model, pred_model, train_dataset, d, normalise_weights, offset, case, sample_range,prefix_name='test'):
    ...
    for sample_size in sample_range:
        ...
        for sample_iter in range(sample_size):        
            ...
            print(z.shape)
            lof_score_val = lof_score_func(z)
...
def compute_eval_metrics_adult(immutables, methods, base_model_dir, encoded_size, pred_model, val_dataset, d, normalise_weights, mad_feature_weights, div_case, case, sample_range, filename,prefix_name='samples' ):   
    ...
    for key in methods.keys():
        ...
        if immutables:
            fcx_vae = FCX_VAE( len(d.encoded_feature_names)-11, encoded_size, d,-11 )
        else:
    ...
    os.makedirs('results/folktables_adult', exist_ok=True)
    plt.savefig('results/folktables_adult/'+filename+'.jpg')
    plt.show()
    return final_res
