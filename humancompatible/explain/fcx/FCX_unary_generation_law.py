...
#from scripts.vae_model_simple import CF_VAE
...
def train_constraint_loss(model, train_dataset, optimizer, normalise_weights, validity_reg, constraint_reg, margin, epochs=1000, batch_size=1024,adj_matrix=None):
    ...
    for train_x in enumerate(train_dataset):
        ...
        temp_lof_loss = criterion(x_pred).item()
...
def train_unary_fcx_vae(
    ...
    vae_train_dataset = np.load(os.path.join(base_data_dir, dataset_name, dataset_name + '-train-set.npy'))
    vae_val_dataset   = np.load(os.path.join(base_data_dir, dataset_name, dataset_name + '-val-set.npy'))
    ...
    with open(os.path.join(base_data_dir, dataset_name, dataset_name + '-normalise_weights.json')) as f:
        normalise_weights = json.load(f)
    normalise_weights = {int(k):v for k,v in normalise_weights.items()}

    with open(os.path.join(base_data_dir, dataset_name, dataset_name + '-mad.json')) as f:
    ...
    dd_check = pd.read_csv(os.path.join(base_data_dir, dataset_name, 'law-train-set_check.csv'))
    adj      = pd.read_csv(os.path.join(base_data_dir, dataset_name, 'law_causal_graph_adjacency_matrix.csv'), index_col=0)
    ...
    # Detect cycles and remove last edge
    G = nx.from_numpy_matrix(adj.values, create_using=nx.DiGraph)
    try:
    ...
    for ep in range(epochs):
        ...
        print('----Epoch:', ep, 'Loss:', loss_val[-1], 'Best:', best_loss)
