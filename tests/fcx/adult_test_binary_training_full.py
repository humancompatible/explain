...
import humancompatible.explain.fcx.FCX_binary_generation_adult as binary_mod
from humancompatible.explain.fcx.FCX_binary_generation_adult import (
    compute_loss,
...
def test_reconstruction_penalty_monotonicity(dummy_data, dummy_model):
    x, y, adj, normals, pm = dummy_data
    # change immutable part so only sparsity matters
...
def test_lofloss_integration(dummy_data, dummy_model):
    ...
    loss = train_constraint_loss(dummy_model, np.zeros((8,9),dtype=np.float32),
                                 ...
                                 adj_matrix=adj, ed_dict={4:1.0}, pred_model=pm)
...
def test_parameter_updates(dummy_data):
    ...
    _ = train_constraint_loss(tiny, np.zeros((4,6),dtype=np.float32),
                              ...
                              adj_matrix=torch.eye(5), ed_dict={}, pred_model=pm)
...
def test_train_binary_fcx_vae_integration(monkeypatch, tmp_path):
    ...
    # stub data & models
    monkeypatch.setattr(binary_mod, 'load_adult_income_dataset', lambda: pd.DataFrame({'age':[1],'hours_per_week':[2],'income':[0]}))
    monkeypatch.setattr(binary_mod, 'DataLoader', lambda params: MagicMock(get_indexes_of_features_to_vary=lambda x:[0,1], encoded_feature_names=list(range(30))))
    monkeypatch.setattr(np, 'load', lambda p: np.zeros((1,9)))
    class StubBB:
