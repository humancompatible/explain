...
import humancompatible.explain.fcx.evaluate_unary_folktables_adult as eval_mod
from humancompatible.explain.fcx.evaluate_unary_folktables_adult import evaluate_folktables_adult_unary


# ─── 6) Monkey‑patch external dependencies ─────────────────────────────────
@pytest.fixture(autouse=True)
def patch_everything(monkeypatch):
    # 6.1) Stub pd.read_csv so that drop(columns=['POBP','RELP']) works
    ...
    class DL:
        def __init__(self, params):
            self.encoded_feature_names = ['age','hours_per_week','education','workclass','marital_status','occupation']
        def get_indexes_of_features_to_vary(self, feats):
            # return all six for simplicity
            return list(range(6))
    monkeypatch.setattr(eval_mod, 'DataLoader', DL)

    # 6.3) np.load → toy test‐set with an extra label column
    arr = np.hstack((np.zeros((5,6)), np.zeros((5,1))))
    ...
    normalise = {'0':[0.0,1.0],'1':[0.0,1.0],'2':[0.0,1.0],'3':[0.0,1.0],'4':[0.0,1.0],'5':[0.0,1.0]}
    ...
    # 6.7) FCX_VAE stub (unused by evaluation)
    class VAE:
...
# ─── 7) Test evaluate_folktables_adult_unary ────────────────────────────────
def test_evaluate_folktables_adult_unary_mapping():
    out = evaluate_folktables_adult_unary(
        base_data_dir='any/',
        base_model_dir='any/',
